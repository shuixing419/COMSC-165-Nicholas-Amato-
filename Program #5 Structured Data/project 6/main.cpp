...
int main()
{
    ...
    Timer timer2(5, false, "Time's up!"); // A timer that counts down from 5 seconds
...
void Timer::start()
{
    int count = countUp ? 0 : seconds; // Initialize count based on countUp
    int step = countUp ? 1 : -1; // Set step based on countUp

    while (count <= seconds || count > 0)
    {
        cout << count << " ";

   /*   if ((countUp && count >= seconds) || (!countUp && count <= 0))
        {
